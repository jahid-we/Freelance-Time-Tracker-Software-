...
const handleDelete = async () => {
    ...
    try {
        const response = await axios.delete("/delete-all-clients");
        if (response.status === 200) {
            emit("deleted");
            alert("All contacts deleted successfully!");
        } else {
            console.error("Error deleting contacts:", response.data.data);
        }
    } catch (error) {
        if (error.response && error.response.status === 404) {
            alert("No contacts found to delete.");
            emit("cancel");
        } else {
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 rounded-3 shadow">
                ...
                <div class="modal-body text-center">
                    <p class="mb-2">
                        Do you really want to delete All Clients?
                    </p>
