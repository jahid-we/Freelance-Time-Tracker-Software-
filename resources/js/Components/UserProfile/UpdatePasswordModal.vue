...
const handleUpdate = async () => {
  if (
    ...
    return;
  }else if(passwordForm.value.new_password !== passwordForm.value.new_password_confirmation) {
  ...
  try {
    const response = await axios.post("/update-user-password", passwordForm.value);
    ...
    alert(
      error.response?.data?.data || "Failed to update password. Please try again."
    );
  } finally {
